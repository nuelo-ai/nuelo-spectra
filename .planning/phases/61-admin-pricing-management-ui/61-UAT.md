...
number: 4
name: Config → Consumer Plans API
expected: |
  GET /subscriptions/plans returns plan list dynamically built from YAML.
  On Demand: "No monthly commitment", "Purchase credits as needed", "Full feature access", "Up to 3 active collections".
  Standard: "100 credits/month", "Priority support", "Full feature access", "Up to 5 active collections" at DB override price.
  Premium: "500 credits/month" + 4 config features at DB override price.
  free_trial and internal tiers NOT shown.
...
expected: /billing-settings page shows 3 card sections. Subscription Pricing card shows Standard at $18.99 and Premium at $44.99 with config default hints ($29.00 and $79.00). Credit Packages card shows all 3 packages with correct prices and credit amounts. Monetization card shows toggle and Stripe readiness.
result: [pending]

### 7. Consumer UI — Plan Selection
expected: Plan selection page (or component) renders On Demand, Standard ($18.99/mo), and Premium ($44.99/mo) with feature bullets matching the YAML features lists plus dynamically generated credits/month and collections bullets. free_trial and internal tiers do NOT appear.
result: [pending]
...
### 9. Reset to Defaults
expected: Admin clicks Reset to Defaults on subscriptions + password confirmation. Standard reverts to $29.00, Premium reverts to $79.00 (matching YAML price_cents). Config default hints disappear or match current values. Consumer API reflects the reset prices.
result: [pending]
